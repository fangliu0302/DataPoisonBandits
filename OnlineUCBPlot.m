...
run = 1;
for Delta = DeltaList
    filename = ['OnlineUCBACA_sigma=',num2str(sigma),'_Delta=',num2str(Delta),'.mat'];
    load(filename)
    cumCost = zeros(T,1);
    ...
    filename = ['OnlineUCBNIPS_sigma=',num2str(sigma),'_Delta=',num2str(Delta),'.mat'];
    load(filename)
    cumCost = zeros(T,1);
    cumCost(1) = Cost(1)/outloop;
    for t = 2:T
...
filename = ['OnlineUCBWithout_sigma=',num2str(sigma),'_Delta=',num2str(Delta),'.mat'];
load(filename)
cumPlay = zeros(T,1);
cumPlay(1) = Target(1)/outloop;
for t = 2:T
