...
delta0Para = sigma;%sigma*sqrt(log(T));
...
for out = 1:outloop
    ...
    for t = 1:T
        ...
        if t<= K
            ...
            att = - mask(dec) * max(0,cnt(dec)*avg(dec)+attSum(dec)-cnt(dec)*(avgAtt(K)-2*beta(cnt(dec))-delta0Para));
        end
        % update average + attack
        avgAtt(dec) = (avgAtt(dec)*(cnt(dec)-1) + reward + att)/cnt(dec);
        attSum(dec) = attSum(dec)+att;
        % For plot
        Target(t) = Target(t) + ind(dec);
        Cost(t) = Cost(t) + abs(att);
    end
end
save(['OnlineUCBNIPS_sigma=',num2str(sigma),'_Delta=',num2str(mean(1)),'.mat'])
toc