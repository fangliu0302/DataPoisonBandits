...
for out = 1:outloop
    ...
    for t = 1:T
        if t<= K
            ...
            [val, in] = max(avg+3*sigma*sqrt(log(t)./cnt));
            dec = in(1);
        end
    ...
    b = ones(K-1,1)*avg(K);
    for i = 1:(K-1)
        b(i) = b(i) - avg(i) - xi + 3*sigma*sqrt(log(T+1)/cnt(K)) - 3*sigma*sqrt(log(T+1)/cnt(i));
    end
    x = quadprog(H,[],A,b);
    ...
    [val,in] = max(avg + C * x +3*sigma*sqrt(log(t)./cnt));
    dec = in(1);
    success = success + ind(dec);
end
save(['OfflineUCB_sigma=',num2str(sigma),'.mat'])
toc